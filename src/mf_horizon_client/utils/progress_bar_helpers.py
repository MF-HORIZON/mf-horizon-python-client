...
def update_single_pipeline_status(
    pipelines: List[Pipeline], progress_bars: List[tqdm], compute_status: dict
) -> None:
    """
    ...
    for pipeline, progress_bar in zip(pipelines, progress_bars):
        ...
        if running_stage:
            progress_bar.set_postfix(
                {
                    "n_scheduled_tasks": compute_status[
                        "num_unfinished_run_tasks_for_user"
                    ],
            ...
            loc = (
                pipeline.stages.index(pipeline.last_completed_stage) + 1
        ...
        if pipeline.is_complete:
            progress_bar.set_description(
                f" \u2714 Complete Pipeline {pipeline.summary.id_}"
            )
            progress_bar.set_postfix({})
