...
pipeline = pipeline_interface.create_pipeline(dataset_id=dataset.summary.id_, blueprint=BlueprintType.fast_forecasting, name="ST")

cast(ProblemSpecificationConfig, pipeline.stages[0].config).horizons = [1]

pipeline_interface.update_config(pipeline_id=pipeline.summary.id_, stage_id=pipeline.stages[0].id_, config=pipeline.stages[0].config)
